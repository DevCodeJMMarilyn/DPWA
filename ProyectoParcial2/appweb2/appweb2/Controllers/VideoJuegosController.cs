...
namespace appweb2.Controllers
{
    public class VideoJuegosController : Controller
    {
        ...
        public async Task<IActionResult> Create(VideoJuego juego, IFormFile archivoImagen)
        {
            ...
            juego.Titulo = juego.Titulo?.Trim() ?? string.Empty;
            juego.Descripcion = string.IsNullOrWhiteSpace(juego.Descripcion) ? null : juego.Descripcion.Trim();

            if (string.IsNullOrWhiteSpace(juego.Titulo))
                ModelState.AddModelError(nameof(VideoJuego.Titulo), "El título es obligatorio.");
        ...
        private void ValidarPromocion(VideoJuego juego)
        {
            ...
            if (!juego.PrecioPromocion.HasValue)
            {
                ModelState.AddModelError(nameof(VideoJuego.PrecioPromocion), "Debes ingresar un precio de promoción.");
                return;
            }

            if (juego.PrecioPromocion.Value <= 0)
            {
                ModelState.AddModelError(nameof(VideoJuego.PrecioPromocion), "El precio de promoción debe ser mayor que cero.");
                return;
            }

            if (juego.PrecioPromocion.Value >= juego.Precio)
            {
